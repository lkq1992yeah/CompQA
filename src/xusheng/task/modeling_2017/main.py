...
class PathPlanner(object):
    ...
    def run_phase_2(self, sudden_change=None):
        ...
        if sudden_change is not None:
            f_starts[self.car_name_idx_map['C3']] = sudden_change[0]
            f_starts[self.car_name_idx_map['C2']] = sudden_change[1]
            f_starts[self.car_name_idx_map['C11']] = sudden_change[2]
    ...
    def run_phase_3(self):
        ...
        queue_z = dict()
        for car_idx, [st_idx, des_idx, path] in self.phase_2_ret.items():
        ...
        for (car_idx, _) in priority_list:
            ...
            for (f_idx, time) in cands:
                if f_idx not in self.visited_fs:
    ...
    def check_conflict(self, ret_dict):
        ...
        for node_interval, time_intervals in road_detail.items():
            ...
            if self.is_main_road(node_interval[0], node_interval[1]):
                continue
            backward_node_interval = (node_interval[1], node_interval[0])
            if backward_node_interval in road_detail:
                print("[log] Find backwards!")
        if len(conflicts) == 0:
    ...
    def summarize(self):
        ...
        for car_idx in range(24):
            ...
            start += path_1[0][1][1]
            ret.append("%.1f" % path_1[0][1][1])

            for i in range(1, len(path_1)-1):
                ret.append("%s\t%.1f\t%.1f" % (self.mapA.idx_name_map[path_1[i][0]],
                                               path_1[i][1][0], path_1[i][1][1]))
            ret.append("%s\t%.1f" % (self.mapA.idx_name_map[path_1[-1][0]], path_1[-1][1][0]))

            path_2 = self.phase_2_ret[car_idx][2]
            ret.append("%.1f" % path_2[0][1][1])
            for i in range(1, len(path_2) - 1):
                ret.append("%s\t%.1f\t%.1f" % (self.mapA.idx_name_map[path_2[i][0]],
                                               path_2[i][1][0], path_2[i][1][1]))
            ret.append("%s\t%.1f" % (self.mapA.idx_name_map[path_2[-1][0]], path_2[-1][1][0]))

            path_3 = self.phase_3_ret[car_idx][2]
            wait += path_3[0][1][1] - path_2[-1][1][0]
            ret.append("%.1f" % path_3[0][1][1])
            for i in range(1, len(path_3) - 1):
                ret.append("%s\t%.1f\t%.1f" % (self.mapA.idx_name_map[path_3[i][0]],
                                               path_3[i][1][0], path_3[i][1][1]))
            ret.append("%s\t%.1f" % (self.mapA.idx_name_map[path_3[-1][0]], path_3[-1][1][0]))
            total += path_3[-1][1][0]
            # print("\t".join(ret))
            ...
            car_path= [self.mapA.idx_name_map[x[0]] for x in path_1]
            car_path_2 = [self.mapA.idx_name_map[x[0]] for x in path_2]
            car_path_3 = [self.mapA.idx_name_map[x[0]] for x in path_3]

            car_path.extend(car_path_2[1:])
            car_path.extend(car_path_3[1:])
            for node in car_path:
                if node not in node_cnt:
                    node_cnt[node] = 0
                node_cnt[node] += 1

            print("[%s] %s" % (self.car_idx_name_map[car_idx], " -> ".join(car_path)))

        print("\n")
        node_cnt = [(key, val) for key, val in node_cnt.items()]
        node_cnt.sort(lambda a, b: -cmp(a[1], b[1]))
        for node_name, cnt in node_cnt:
            print("%s\t%d" % (node_name, cnt))
        print("\n")
        # car_avg.sort(lambda a, b: -cmp(a[1], b[1]))
        for (name, time) in car_avg:
