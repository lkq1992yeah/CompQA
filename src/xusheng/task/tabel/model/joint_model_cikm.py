...
class JointModel(object):
    ...
    def _get_nonjoint_weights(self, setting):
        if setting == "trans":
    ...
    def _build_cell_feature(self, cells=None, entities=None):
        ...
        self.vec_ONE_single = self.transfer(tf.add(tf.matmul(tf.concat([self.cell_vec,
                                                                        entities], 1),  # change: self.ent_vec
                                                             self.weights['w1']),
                                                   self.weights['b1']))
    ...
    # OLD/discard
    def _build_context_feature_with_cnn(self):
        # CNN layer need to have shape of dimension 4, the first dim. is batch size;
        # contexts' shape: (batch_size*rows*cols, 1, r+c-2, d_w2v)
        conv = conv2d(self.contexts, self.weights['wcx'], self.weights['bcx'])
        pool = maxpool2d(conv, 1, self.length - 3 + 1)  # window size = 3; VALID
        # after pooling, pool should be shape (batch_size*rows*columns, d_vec)
        self.context_vec = tf.reshape(pool, [-1, self.d_w2v])

        # concatenate one context vec with corresponding entity
        # shape: (batch size, rows, columns, d_hidden)
        self.vec_TWO_single = self.transfer(tf.add(tf.matmul(tf.concat([self.context_vec,
                                                                        self.ent_vec], 1),
                                                             self.weights['w2']),
                                                   self.weights['b2']))
        ...
        self.vec_TWO_single = tf.reshape(self.vec_TWO_single, [-1, self.rows, self.columns, self.d_hidden])

        # then average among batch size, reduce dim -> 2, need to mean twice! note the axis = 1
        # vec_TWO has the shape of (batch_size, d_hidden)
        self.vec_TWO = tf.reduce_sum(tf.reduce_sum(self.vec_TWO_single, 1), 1)
