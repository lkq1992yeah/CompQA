...
class WordEmbeddingUtil(object):
    ...
    def __init__(self, wd_emb, dim_emb,
        ...
        self.word_emb_mat_fp = '%s/word_emb.%s_%d.npy' % (emb_dir, wd_emb, dim_emb)
        ...
        self.mid_emb_mat_fp = '%s/mid_emb.%s_%d.npy' % (emb_dir, wd_emb, dim_emb)
    ...
    def load_word_indices(self):
        if self.word_idx_dict is None:
            ...
            with open(self.word_dict_fp, 'rb') as br:
                self.word_idx_dict = cPickle.load(br)
    ...
    def load_mid_indices(self):
        if self.mid_idx_dict is None:
            ...
            with open(self.mid_dict_fp, 'rb') as br:
                self.mid_idx_dict = cPickle.load(br)
    ...
    #     word_idx = self.word_idx_dict.get(word, -1)
    #     if word_idx == -1:
    #         return None
    #     return self.word_emb_matrix[word_idx]
        ...
        for tok, active_idx in active_word_dict.items():
            if tok in self.word_idx_dict:
                ...
                if dep_tok in self.dep_name_dict:       # this is a known dependency label
                    ...
                    filt_dep_name_spt = filter(lambda x: x in self.word_idx_dict, dep_name_spt)
