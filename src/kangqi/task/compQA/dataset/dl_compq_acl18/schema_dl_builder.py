...
def prepare_optm_eval_data(schema_dataset, task_name, neg_f1_ths, neg_max_sample, el_use_type):
    """
    ...
    for scan_idx, q_idx in enumerate(schema_dataset.q_idx_list):
        ...
        dynamic_cand_list = []
        if schema_dataset.dynamic_q_cand_dict is not None:
            dynamic_cand_list = schema_dataset.dynamic_q_cand_dict[q_idx]
        dedup_sc_tups = schema_dedup_for_specific_task(
            smart_cand_list=smart_cand_list,
            dynamic_cand_list=dynamic_cand_list,
            task_name=task_name,
        ...
        # pick_neg_size = len(pick_neg_tups)
        # LogInfo.logs('pos = %d, dyn_neg = %d, fix_neg = %d, pick_neg = %d.',
        #              pos_size, len(dyn_neg_tups), len(fix_neg_tups), pick_neg_size)

        # neg_weight = 0.
        # if pick_neg_size > 0:
        #     neg_weight = min(1., 1. * pos_size / pick_neg_size)
        # # TODO: raise neg_weight if there's only one positive schema?
        # # TODO: or let neg_weight dynamically change based on F1 of positive schema

        """ 180313: Let's make things easier: just set equally weighted. """
        neg_weight = 1.
        # poor_contribution = 20
        # if pick_neg_size > 0:
        #     neg_weight = 1. * min(pick_neg_size, poor_contribution) / pick_neg_size
        #     # Neg-20-20 strategy.
        #     # The main difference is that, negative samples contributes as "poor_contribution" number of data points.
        #     # If neg_max_sample <= 20 (avoid generating too many negatives),
        #     # then each negative data holds weight = 1.
        #     # While in the strategy above, negative sample are dramatically diluted.
...
def build_task_dataloaders(task_name, schema_dataset, compq_mt_model,
                           optm_batch_size, eval_batch_size,
                           neg_f1_ths=0.1, neg_max_sample=20, el_use_type=True):
    ...
    optm_dl = SchemaOptmDataLoader(
        ...
        shuffle=False, batch_size=optm_batch_size*2
