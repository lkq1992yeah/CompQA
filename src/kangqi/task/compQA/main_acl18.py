...
from dataset.dl_compq_acl18.seg_dataloader import SegmentDataLoader
from dataset.dl_compq_acl18.schema_dl_builder import build_task_dataloaders

from model.compq_acl18 import CompqMultiTaskModel

from learner.pyltr.pyltr_worker import ltr_whole_process
from learner.compq_acl18 import \
...
# seg_conll_dataset = SegmentDataset(seg_data_name='conll03',
#                                    q_max_len=q_max_len,
#                                    word_dict=wd_emb_util.wd_idx_dict)
# seg_simpq_dataset = SegmentDataset(seg_data_name='SimpQ',
#                                    q_max_len=q_max_len,
#                                    word_dict=wd_emb_util.wd_idx_dict)
# seg_dl_list = prepare_seg_dataloader(seg_dataset=seg_simpq_dataset,
#                                      optm_batch_size=args.optm_batch_size,
#                                      eval_batch_size=args.eval_batch_size)
# seg_optimizer = SegmentOptimizer(compq_mt_model=compq_mt_model, sess=sess, ob_batch_num=100)
# seg_evaluator = SegmentEvaluator(compq_mt_model=compq_mt_model, sess=sess, ob_batch_num=100)
# LogInfo.logs('Optimizer & Evaluator defined for [seg].')


def prepare_seg_dataloader(seg_dataset, optm_batch_size, eval_batch_size):
    data_size = len(seg_dataset.np_data_list[0])
    train_pos = int(data_size * 0.66667)
    valid_pos = int(data_size * 0.83333)
    seg_optm_dl = SegmentDataLoader(dataset=seg_dataset,
                                    mode='T', shuffle=True,
                                    batch_size=optm_batch_size,
                                    indices_list=range(train_pos))
    seg_eval_train_dl = SegmentDataLoader(dataset=seg_dataset,
                                          mode='T', shuffle=False,
                                          batch_size=eval_batch_size,
                                          indices_list=range(train_pos))
    seg_eval_valid_dl = SegmentDataLoader(dataset=seg_dataset,
                                          mode='v', shuffle=False,
                                          batch_size=eval_batch_size,
                                          indices_list=range(train_pos, valid_pos))
    seg_eval_test_dl = SegmentDataLoader(dataset=seg_dataset,
                                         mode='t', shuffle=False,
                                         batch_size=eval_batch_size,
                                         indices_list=range(valid_pos, data_size))
    return seg_optm_dl, seg_eval_train_dl, seg_eval_valid_dl, seg_eval_test_dl
...
def main(args):
    ...
    for key in ('q_max_len', 'sc_max_len',
                'path_max_len', 'pword_max_len',
    ...
    for epoch in range(start_epoch+1, args.max_epoch+1):
        ...
        for task_name in eval_tasks:
            task_dls_dict[task_name] = build_task_dataloaders(
                task_name=task_name, schema_dataset=schema_dataset, compq_mt_model=compq_mt_model,
                optm_batch_size=args.optm_batch_size, eval_batch_size=args.eval_batch_size,
                el_use_type=el_use_type, neg_f1_ths=args.neg_f1_ths, neg_max_sample=args.neg_max_sample
        ...
        if epoch <= args.pre_train_steps or not full_back_prop:
            for task, task_dl_list, evaluator in [('el', el_dl_list, el_evaluator),
    ...
    # LogInfo.begin_track('Extra evaluation for checking loglik:')
    # extra_data_list, _ = seg_optm_dl.get_batch(batch_idx=0)
    # extra_feed_dict = {tensor: data for tensor, data in zip(compq_mt_model.seg_input_tensors, extra_data_list)}
    # optm_hidden, eval_hidden, optm_logits, eval_logits, optm_loglik, eval_loglik = \
    #     sess.run(fetches=[compq_mt_model.optm_tensor_dict['v_hidden'],
    #                       compq_mt_model.eval_tensor_dict['v_hidden'],
    #                       compq_mt_model.optm_tensor_dict['seg_logits'],
    #                       compq_mt_model.eval_tensor_dict['seg_logits'],
    #                       compq_mt_model.optm_tensor_dict['seg_loglik'],
    #                       compq_mt_model.eval_tensor_dict['seg_loglik']],
    #              feed_dict=extra_feed_dict)
    # np.set_printoptions(threshold=np.nan)
    # LogInfo.logs('========== optm_hidden ============')
    # LogInfo.logs(optm_hidden)
    # LogInfo.logs('========== eval_hidden ============')
    # LogInfo.logs(eval_hidden)
    # LogInfo.logs('========== optm_logits ============')
    # LogInfo.logs(optm_logits)
    # LogInfo.logs('========== eval_logits ============')
    # LogInfo.logs(eval_logits)
    # LogInfo.logs('========== optm_loglik ============')
    # LogInfo.logs(optm_loglik)
    # LogInfo.logs('========== eval_loglik ============')
    # LogInfo.logs(eval_loglik)
